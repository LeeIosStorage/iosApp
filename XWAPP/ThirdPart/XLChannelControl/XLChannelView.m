...
//横向和纵向的间距
static CGFloat CellMarginX = 18.0f;
...
-(void)buildUI{
    ...
    flowLayout.itemSize = CGSizeMake(cellWidth,40);
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    headerView.rightBtn.selected = hiddenBtn;
    [headerView changeStatusBlock:^(BOOL selected) {
        self->hiddenBtn = selected;
        [collectionView reloadData];
    }];
    if (indexPath.section == 0) {
        headerView.title = @"已选频道";
        headerView.rightBtn.hidden = NO;
        headerView.subTitle = @"拖动可以排序";
    }else{
        headerView.title = @"更多频道";
